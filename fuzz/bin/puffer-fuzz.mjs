...
const defaultFakeDaemonPath = path.resolve(fuzzRoot, "..", "apps", "puffer-desktop", "tests", "support", "fakeDaemon.ts");
...
Options:
  --manifest <path>   Default: fuzz/manifests/puffer-ui.json
  --seed-dir <path>   Default: fuzz/seeds
  --adapter <path>    Default: fuzz/adapters/playwright-actions.json
  --ledger <path>     Default: fuzz/coverage-ledger.json
...
async function main() {
  ...
  if (command === "top-cases") {
    ...
    const run = await readJson(args.input);
    const selection = selectTopCases(run, { limit: args.limit });
  ...
  if (command === "replay") {
    ...
    const template = buildReplayTemplate(selected, {
      ...
      fakeDaemonImport: args["fake-daemon-import"] ??
        moduleSpecifier(resolvedOutputPath, path.resolve(fuzzRoot, "..", "apps", "puffer-desktop", "tests", "support", "fakeDaemon"))
    });
