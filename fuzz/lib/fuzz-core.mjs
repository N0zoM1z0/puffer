...
export function formatPlanMarkdown(plan) {
  ...
  for (const row of plan.seeds) {
    ...
    lines.push(`- Command: \`node fuzz/bin/puffer-fuzz.mjs run --seed ${row.seed.id} --iterations ${row.seed.defaultIterations ?? 8} --steps ${row.seed.defaultSteps ?? 12}${profileArg} --out /tmp/puffer-fuzz-${row.seed.id}.json\``);
    lines.push("");
  }
  return `${lines.join("\n")}\n`;
}
...
Workflow:
1. Run \`node fuzz/bin/puffer-fuzz.mjs run --seed ${seed.id} --iterations ${iterations} --steps ${steps} --out /tmp/puffer-fuzz-${seed.id}.json\`.
2. Run \`node fuzz/bin/puffer-fuzz.mjs report --input /tmp/puffer-fuzz-${seed.id}.json --out /tmp/puffer-fuzz-${seed.id}.md\`.
3. Pick the highest-value generated case that exercises a stale response, reconnect, duplicate submit, or state switch.
4. Replay it against the existing Playwright fake daemon harness.
5. If it reproduces a product bug, shrink the steps to the smallest stable sequence.
6. Record the finding under \`bugs/\` with trigger, expected, actual, impact, and regression target.
7. Only after a confirmed bug exists, implement the fix and add a deterministic \`*.spec.ts\` regression.
...
- Every accepted fix must include a regression test and a concise component spec.
`;
}
