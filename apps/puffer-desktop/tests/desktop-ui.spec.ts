...
test("late Browser navigation failures do not mark the active tab", async ({ page }) => {
  const daemon = new FakeDaemon();
  await daemon.install(page);
  await daemon.open(page);
  ...
  daemon.failNext("browser_navigate", "navigation failed");
  await page.getByLabel("URL").fill("broken.example");
  await page.getByLabel("URL").press("Enter");
  await daemon.waitForRequest("browser_navigate", (candidate) =>
...
test("late Browser close fallback does not close a different agent", async ({ page }) => {
  const daemon = new FakeDaemon({
    sessions: [
      {
        sessionId: "session-alpha-browser",
  ...
  daemon.failNext("browser_agent", "alpha tab close failed");
